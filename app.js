...
function animate() {
  ...
  if (!processing && operations.length > 0) {
    ...
    if (action.swap) {
      framesPerSecond
        ? cols[i].moveTo(cols[j], framesPerSecond)
        : cols[i].moveTo(cols[j]);
      framesPerSecond
        ? cols[j].moveTo(cols[i], framesPerSecond)
...
function mergeSort(arg) {
  ...
  function merge(start, mid, end) {
    ...
    while (i < left.length && j < right.length) {
      if (left[i] <= right[j]) {
        // bring left[i] to position k using swaps
        let from = array.indexOf(left[i], k);
        for (let x = from; x > k; x--) {
          actions.push({ index: [x, x - 1], swap: true });
          [array[x], array[x - 1]] = [array[x - 1], array[x]];
        }
        i++;
      } else {
        let from = array.indexOf(right[j], k);
        for (let x = from; x > k; x--) {
          actions.push({ index: [x, x - 1], swap: true });
          [array[x], array[x - 1]] = [array[x - 1], array[x]];
        }
